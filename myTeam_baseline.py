...
class PositionSearchProblem:
  """
  It is the ancestor class foår all the search problem class.
...
class myAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    if not self.current_target == None:
      ...
      # record the agent and its target
      ...
      for position in foodGrid.asList():
        # the agent's target shouldn't be same as teammate's
    ...
    while not myQ.isEmpty():
      ...
      for successor, action, _ in problem.getSuccessors(currentState):
        if successor not in visited:
          myQ.push((successor, path + [action]))
